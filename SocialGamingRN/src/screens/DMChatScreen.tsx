...
import { createDMSocket } from '../lib/ws';
...
export default function DMChatScreen({ route }: Props) {
  const { peerId } = route.params;
  const myId = useAuth((s) => s.user?.id);
  const [messages, setMessages] = useState<ChatItem[]>([]);
  const [input, setInput] = useState('');
  const [sending, setSending] = useState(false);
  const socketRef = useRef<ReturnType<typeof createDMSocket> | null>(null);

  useEffect(() => {
    const sock = createDMSocket(peerId);
  ...
  const send = () => {
    ...
    if (!text) return;
    setSending(true);
    const ok = socketRef.current?.sendJSON({ type: 'chat', content: text, ts: Date.now(), to: peerId });
    if (ok) setInput('');
  };
